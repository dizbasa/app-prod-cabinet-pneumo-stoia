...
class ErrorBoundary extends React.Component {
  ...
  render(){
    if(this.state.error)return<div style={{padding:40,fontFamily:'monospace',background:'#fff',color:'red',whiteSpace:'pre-wrap'}}>
      <h2>App Error (visible in production)</h2>
...
function SBadge({status}){return<span style={{display:'inline-block',padding:'2px 10px',background:STATUS_COLOR[status]+'22',color:STATUS_COLOR[status],border:`1px solid ${STATUS_COLOR[status]}44`,fontFamily:"'DM Sans',sans-serif",fontSize:11,borderRadius:4}}>{STATUS_LABEL[status]}</span>}
...
function AvailPicker({days,start,end,onD,onS,onE}){
  ...
  return<div style={{display:'flex',flexDirection:'column',gap:10}}>
    <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
      {DAYS_FULL.map(d=><div key={d} onClick={()=>tog(d)} style={{padding:'5px 12px',border:`1.5px solid ${sel.includes(d)?'#1A3A2A':'#D5CFCA'}`,background:sel.includes(d)?'#1A3A2A':'transparent',color:sel.includes(d)?'#F5F0E8':'#1A3A2A',fontFamily:"'DM Sans',sans-serif",fontSize:12,cursor:'pointer',borderRadius:4}}>{d}</div>)}
    </div>
...
function SafeModal({onClose,children,width=560}){
  ...
  >
    <div style={{background:'#fff',width:`min(${width}px,calc(100vw - 24px))`,maxHeight:'90dvh',overflowY:'auto',borderRadius:4,position:'relative',margin:'auto'}} onMouseDown={e=>e.stopPropagation()}>
...
function ConfirmModal({title,message,onConfirm,onCancel,danger=true}){
  return<SafeModal onClose={onCancel} width={420}>
    <div style={{textAlign:'center',padding:'8px 0'}}>
      <div style={{width:48,height:48,borderRadius:'50%',background:danger?'#FFF0F0':'#F0F7F0',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 16px',fontSize:22}}>
        {danger?'⚠':'?'}
      </div>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,color:'#1A3A2A',marginBottom:10}}>{title}</div>
      <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,color:'#6B7A6E',lineHeight:1.6,marginBottom:24}}>{message}</div>
      <div style={{display:'flex',gap:10,justifyContent:'center'}}>
        <button className="btn-outline" style={{color:'#1A3A2A'}} onClick={onCancel}>Anulează</button>
        <button onClick={onConfirm} style={{background:danger?'#8B3A3A':'#2D5A3F',color:'white',border:'none',padding:'10px 24px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:12,letterSpacing:'.1em',textTransform:'uppercase',borderRadius:4}}>
          {danger?'Șterge':'Confirmă'}
        </button>
      </div>
    </div>
  </SafeModal>
...
// Helper hook for confirm dialog
function useConfirm(){
  const [state,setState]=useState(null)
  const confirm=(title,message,onConfirm)=>setState({title,message,onConfirm})
  const modal=state?<ConfirmModal title={state.title} message={state.message} onConfirm={()=>{state.onConfirm();setState(null)}} onCancel={()=>setState(null)}/>:null
...
function DocPanel({entityType,entityId,uploadedBy,canAdd=true,canDelete=true}){
  ...
  return<div>
    ...
    :docs.map(d=><div key={d.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 12px',background:'#F5F0E8',marginBottom:4,borderRadius:4}}>
...
function TemplatesTab({type,label,canEdit,canDelete}){
  ...
  const [form,setForm]=useState({name:'',notes:'',severity:'',code:'',dosage:'',duration:''})
  ...
  const save=async()=>{
    ...
    await fetch('/api/admin/templates',{method:editId?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})
    await load();setAdding(false);setEditId(null);setForm({name:'',notes:'',severity:'',code:'',dosage:'',duration:''})
  }
  ...
  const startEdit=it=>{setEditId(it.id);setForm({name:it.name,notes:it.notes||'',severity:it.severity||'',code:it.code||'',dosage:it.dosage||'',duration:it.duration||''});setAdding(false)}

  const filtered=items
    .filter(it=>!search||(it.name||'').toLowerCase().includes(search.toLowerCase())||(it.notes||'').toLowerCase().includes(search.toLowerCase()))
  ...
  return<div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10,flexWrap:'wrap',gap:8}}>
      ...
      <div style={{display:'flex',gap:8,alignItems:'center'}}>
        <Inp placeholder="Caută..." value={search} onChange={e=>setSearch(e.target.value)} style={{width:160,padding:'6px 10px'}}/>
        <button onClick={()=>setSortDir(d=>d==='asc'?'desc':'asc')} className="btn-outline" style={{color:'#1A3A2A',padding:'6px 10px'}}>{sortDir==='asc'?'A→Z':'Z→A'}</button>
        {canEdit&&<button className="btn-primary" style={{padding:'6px 14px',fontSize:11}} onClick={()=>{setAdding(true);setEditId(null);setForm({name:'',notes:'',severity:'',code:'',dosage:'',duration:''})}}>+ Adaugă</button>}
      </div>
    </div>
    {(adding||editId)&&<div style={{background:'#F5F0E8',border:'1px solid #E5DFD3',padding:14,marginBottom:12,borderRadius:6}}>
      <div style={{display:'flex',flexDirection:'column',gap:8}}>
        ...
        <div style={{display:'flex',gap:8}}>
          <button className="btn-primary" style={{padding:'7px 14px',fontSize:11}} onClick={save}>Salvează</button>
          <button className="btn-outline" style={{color:'#1A3A2A',padding:'7px 14px',fontSize:11}} onClick={()=>{setAdding(false);setEditId(null)}}>Anulează</button>
        </div>
      </div>
    </div>}
    <div style={{background:'#fff',border:'1px solid #E5DFD3',borderRadius:6}}>
      {filtered.length===0
        ?<div style={{padding:'14px',fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E',textAlign:'center'}}>Nicio intrare{search?' pentru căutarea curentă':''}.</div>
        :filtered.map(it=><div key={it.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 16px',borderBottom:'1px solid #F0EBE0'}}>
          <div>
            <div style={{fontFamily:"'DM Sans',sans-serif",color:'#1A3A2A',fontWeight:500,fontSize:13}}>{it.name}</div>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#6B7A6E',display:'flex',gap:8,flexWrap:'wrap',marginTop:2}}>
              {it.severity&&<span>Severitate: {it.severity}</span>}
              {it.code&&<span>ICD: {it.code}</span>}
              {it.dosage&&<span>Doză: {it.dosage}</span>}
              {it.duration&&<span>Durată: {it.duration}</span>}
              {it.notes&&<span>{it.notes}</span>}
            </div>
          </div>
          {canEdit&&<div style={{display:'flex',gap:6}}>
            <button className="btn-outline" style={{color:'#1A3A2A',padding:'3px 8px',fontSize:10}} onClick={()=>startEdit(it)}>Edit</button>
  ...
  return<SafeModal onClose={onClose} width={520}>
    ...
    {tpls.length>0&&<>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:20}}>
        {tpls.map(t=><div key={t.id} onClick={()=>fromTemplate(t)} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 14px',background:'#F5F0E8',border:'1px solid #E5DFD3',cursor:'pointer',borderRadius:4}}>
...
function VisitCard({visit,canWrite,medics,services,onDelete,onRefresh,templates}){
  ...
  const Section=({title,color,children,onAdd,addLabel})=><div style={{marginBottom:20}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
      ...
      {canWrite&&onAdd&&<button onClick={onAdd} style={{background:'none',border:`1px solid ${color}`,padding:'3px 10px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:11,color,borderRadius:4}}>{addLabel||'+ Adaugă'}</button>}
    </div>
    {children}
  </div>

  const Chip=({label,sub,onDel})=><div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 12px',background:'#fff',border:'1px solid #E5DFD3',marginBottom:6,borderRadius:4}}>
    <div style={{flex:1}}>
    ...
    <div style={{padding:'20px 24px'}}>
      ...
      <div style={{marginTop:16,paddingTop:16,borderTop:'1px solid #F0EBE0'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
          ...
          {canWrite&&<button onClick={()=>fileRef.current?.click()} disabled={uploading} style={{background:'none',border:'1px solid #5C4A3F',padding:'3px 10px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#5C4A3F',borderRadius:4,display:'flex',alignItems:'center',gap:5}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:8}}>
          {docs.map(d=><div key={d.id} style={{background:'#fff',border:'1px solid #E5DFD3',padding:'10px 12px',display:'flex',justifyContent:'space-between',alignItems:'center',borderRadius:4}}>
      ...
      {canWrite&&<div style={{marginTop:16,display:'flex',gap:8,justifyContent:'flex-end'}}>
        <button onClick={()=>onDelete(v.id)} style={{background:'none',border:'1px solid #E5A0A0',padding:'5px 14px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#8B3A3A',borderRadius:4}}>Șterge vizita</button>
      </div>}
    </div>
  </>
}
...
function VisitModal({patient,medics,services,currentMedicId,prefillDate,onClose,onSaved,canEditTemplates=false}){
  ...
  const Chip2=({item,type,fields})=><div style={{display:'flex',alignItems:'center',gap:8,padding:'7px 12px',background:'#fff',border:'1px solid #E5DFD3',marginBottom:5,borderRadius:4}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:16}}>
      ...
      <div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
          ...
          <button onClick={()=>setSubModal('symptom')} style={{background:'none',border:'1px solid #C9A84C',padding:'3px 10px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#C9A84C',borderRadius:4}}>+ Adaugă</button>
        </div>
      ...
      <div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
          ...
          <button onClick={()=>setSubModal('diagnostic')} style={{background:'none',border:'1px solid #2D5A3F',padding:'3px 10px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#2D5A3F',borderRadius:4}}>+ Adaugă</button>
        </div>
      ...
      <div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
          ...
          <button onClick={()=>setSubModal('treatment')} style={{background:'none',border:'1px solid #3F4A5C',padding:'3px 10px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#3F4A5C',borderRadius:4}}>+ Adaugă</button>
        </div>
  ...
  return<div style={{maxWidth:940,margin:'0 auto',padding:'32px clamp(16px,5vw,40px)'}}>
    ...
    <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:24,flexWrap:'wrap'}}>
      ...
      <div style={{display:'flex',gap:8,marginLeft:'auto'}}>
        <button className="btn-outline" style={{color:'#6B7A6E',padding:'6px 14px',fontSize:11}} onClick={()=>{
          if(!visits.length)return
          const rows=visits.map(v=>({Data:v.visit_date,Medic:v.medic_name||'',Note:v.notes||'',Simptome:(v.symptoms||[]).map(s=>s.name).join('; '),Diagnostice:(v.diagnostics||[]).map(d=>d.name+(d.code?` [${d.code}]`:'')).join('; '),Tratamente:(v.treatments||[]).map(t=>t.name+(t.dosage?` ${t.dosage}`:'')+(t.duration?` ${t.duration}`:'')).join('; '),Servicii:(v.services||[]).map(s=>s.service_name).join('; ')}))
          const keys=Object.keys(rows[0])
          const csv=[keys.join(','),...rows.map(r=>keys.map(k=>`"${String(r[k]||'').replace(/"/g,'""')}"`).join(','))].join('\n')
          const blob=new Blob(['\uFEFF'+csv],{type:'text/csv'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=patient.name+'_istoric.csv';a.click();URL.revokeObjectURL(url)
        }}>↓ Export CSV</button>
        {canWrite&&<button className="btn-primary" onClick={()=>setShowVisitModal(true)}>+ Vizită Nouă</button>}
      </div>
    </div>
    ...
    :visits.map(v=><div key={v.id} style={{background:'#fff',border:'1px solid #E5DFD3',marginBottom:10,borderRadius:4,overflow:'hidden'}}>
...
const MEDIC_FLAG_STRUCTURE = [
  ...
  { cat: 'programari', label: 'Programări', flags: [
    ...
    { flag: 'bookings_reassign_medic',     label: 'Poate schimba Medicul unei Programări (și acces Panou)' },
  ]},
  ...
  { cat: 'export', label: 'Export', flags: [
    { flag: 'export_patients',   label: 'Poate exporta Pacienți' },
    { flag: 'export_bookings',   label: 'Poate exporta Programări' },
    { flag: 'export_nomenclator', label: 'Poate exporta Nomenclatorul' },
    { flag: 'export_visits',     label: 'Poate exporta Vizitele unui Pacient' },
  ]},
  { cat: 'asistenti', label: 'Asistenți', flags: [
    { flag: 'assistants_view',   label: 'Poate vedea secțiunea Asistenți' },
    { flag: 'assistants_manage', label: 'Poate gestiona Asistenți' },
  ]},
]
...
function MedicFlagsPanel({ medicId, pendingFlags, onPendingChange }) {
  ...
  const isOpen = cat => openGroups[cat] !== false
  const toggleCat = cat => setOpenGroups(p => ({ ...p, [cat]: !isOpen(cat) }))
  ...
  if (loading) return <div style={{ padding: '16px', fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: '#6B7A6E' }}>Se încarcă...</div>

  return (
    <div style={{ border: '1px solid #E5DFD3', borderRadius: 4, overflow: 'hidden' }}>
      {MEDIC_FLAG_STRUCTURE.map(group => {
        const allOn = groupAllOn(group)
        const allOff = groupAllOff(group)
        const open = isOpen(group.cat)
        return (
          <div key={group.cat} style={{ borderBottom: '1px solid #E5DFD3' }}>
            <div
              onClick={() => toggleCat(group.cat)}
              style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 16px', background: '#F5F0E8', cursor: 'pointer', userSelect: 'none' }}
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }} onClick={e => e.stopPropagation()}>
                <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 10, color: allOn ? '#2D5A3F' : allOff ? '#8B3A3A' : '#C9A84C' }}>{allOn ? 'Toate ON' : allOff ? 'Toate OFF' : 'Parțial'}</span>
                <Tog val={allOn} onCl={() => toggleGroup(group, !allOn)} />
                <span style={{ color: '#6B7A6E', fontSize: 12, marginLeft: 4, width: 14, textAlign: 'center' }}>{open ? '▲' : '▼'}</span>
              </div>
            </div>
            {open && group.flags.map(f => (
              <div key={f.flag} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 16px 9px 28px', borderTop: '1px solid #F5F0E8' }}>
...
function PatientFlagsPanel({ patientId, pendingFlags, onPendingChange }) {
  ...
  return (
    <div style={{ border: '1px solid #E5DFD3', borderRadius: 4, overflow: 'hidden' }}>
      {PATIENT_FLAG_STRUCTURE.map(group => {
        const allOn = groupAllOn(group)
        const allOff = groupAllOff(group)
        const open = isOpen(group.cat)
        return (
...
// CRUD Modal
function CrudModal({model,item,onClose,onSave,services=[],currentUserRole=''}){
  const [form,setForm]=useState({...item}||{})
  const [saving,setSaving]=useState(false)
  const [err,setErr]=useState('')
  const [medicSvcs,setMedicSvcs]=useState([])
  const [pendingFlags,setPendingFlags]=useState({})
  const [showPatientFlags,setShowPatientFlags]=useState(false)
  const [showMedicFlags,setShowMedicFlags]=useState(false)
  ...
  return<SafeModal onClose={onClose} width={580}>
    ...
    {model==='medics'&&<>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:12}}>
        <F label="Nume (familie)"><Inp value={form.name||''} onChange={e=>fv('name',e.target.value)}/></F>
        ...
        {currentUserRole==='master'&&<F label="Rol"><Sel value={form.staffRole||'medic'} onChange={e=>fv('staffRole',e.target.value)}><option value="medic">Medic</option><option value="assistant">Asistent Medical</option><option value="admin">Administrator</option></Sel></F>}
      </div>
      ...
      <button type="button" onClick={()=>setShowMedicFlags(true)} style={{background:'none',border:'1.5px solid #1A3A2A',padding:'8px 16px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#1A3A2A',borderRadius:4,marginBottom:10,display:'flex',alignItems:'center',gap:6}}>
        <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="8" cy="8" r="6"/><path d="M8 5v3l2 2"/></svg>
        Permisiuni Medic
      </button>
      {showMedicFlags&&<SafeModal onClose={()=>setShowMedicFlags(false)} width={600}>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,color:'#1A3A2A',marginBottom:16}}>Permisiuni Medic</div>
        <MedicFlagsPanel medicId={form.id||null} pendingFlags={pendingFlags} onPendingChange={setPendingFlags}/>
        <button className="btn-primary" style={{marginTop:16}} onClick={()=>setShowMedicFlags(false)}>Închide</button>
      </SafeModal>}
    </>}
    ...
    {model==='patients'&&<>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:12}}>
        <F label="Nume *"><Inp value={form.name||''} onChange={e=>fv('name',e.target.value)}/></F>
        <F label="Prenume *"><Inp value={form.prenume||''} onChange={e=>fv('prenume',e.target.value)}/></F>
        <F label="CNP"><Inp value={form.cnp||''} onChange={e=>fv('cnp',e.target.value)} maxLength={13}/></F>
        <F label="CID (nr. fișă)"><Inp value={form.cid||''} onChange={e=>fv('cid',e.target.value)}/></F>
        <F label="Sex"><Sel value={form.sex||''} onChange={e=>fv('sex',e.target.value)}><option value="">—</option><option value="M">Masculin</option><option value="F">Feminin</option></Sel></F>
        <F label="Data nașterii"><Inp type="date" value={form.data_nasterii||''} onChange={e=>fv('data_nasterii',e.target.value)}/></F>
        <F label="Data decesului"><Inp type="date" value={form.data_decesului||''} onChange={e=>fv('data_decesului',e.target.value)}/></F>
        <F label="Email"><Inp type="email" placeholder="pacient@email.com" value={form.email||''} onChange={e=>fv('email',e.target.value)}/></F>
        <F label="Telefon"><Inp type="tel" value={form.phone||''} onChange={e=>fv('phone',e.target.value)}/></F>
        <F label="Oraș"><Inp value={form.oras||''} onChange={e=>fv('oras',e.target.value)}/></F>
        <F label="Județ"><Inp value={form.judet||''} onChange={e=>fv('judet',e.target.value)}/></F>
        <F label="Adresă" col="1/-1"><Inp placeholder="Str. Exemplu, nr. 1, ap. 2" value={form.adresa||''} onChange={e=>fv('adresa',e.target.value)}/></F>
        <F label="Note" col="1/-1"><Txt rows={2} value={form.notes||''} onChange={e=>fv('notes',e.target.value)}/></F>
      </div>
      <button type="button" onClick={()=>setShowPatientFlags(true)} style={{background:'none',border:'1.5px solid #3F4A5C',padding:'8px 16px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#3F4A5C',borderRadius:4,marginBottom:10,display:'flex',alignItems:'center',gap:6}}>
        <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="8" cy="8" r="6"/><path d="M8 5v3l2 2"/></svg>
        Permisiuni Pacient
      </button>
      {showPatientFlags&&<SafeModal onClose={()=>setShowPatientFlags(false)} width={600}>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,color:'#1A3A2A',marginBottom:16}}>Permisiuni Pacient</div>
        <PatientFlagsPanel patientId={form.id||null} pendingFlags={pendingFlags} onPendingChange={setPendingFlags}/>
        <button className="btn-primary" style={{marginTop:16}} onClick={()=>setShowPatientFlags(false)}>Închide</button>
      </SafeModal>}
      <div style={{background:'#F5F0E8',border:'1px solid #E5DFD3',padding:14,marginTop:10,borderRadius:6}}>
...
// Flags editor
function FlagsEditor({flags,onToggle,title='Setări Admin',subtitle=''}){
  const cats=[...new Set(flags.map(f=>f.category))]
  const catLabel={medics:'Medici',services:'Servicii',patients:'Pacienți',bookings:'Programări',homepage:'Pagina Principală',visits:'Vizite',analytics:'Prezentare',export:'Export',import:'Import',other:'Altele'}
  const [openCats,setOpenCats]=useState({})
  const isCatOpen=c=>openCats[c]!==false
  return<div>
    ...
    {cats.map(cat=><div key={cat} style={{marginBottom:10}}>
      <div onClick={()=>setOpenCats(p=>({...p,[cat]:!isCatOpen(cat)}))} style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,letterSpacing:'.12em',textTransform:'uppercase',color:'#6B7A6E',marginBottom:4,cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',userSelect:'none'}}>
        <span>{catLabel[cat]||cat}</span><span style={{fontSize:10}}>{isCatOpen(cat)?'▲':'▼'}</span>
      </div>
      {isCatOpen(cat)&&<div style={{background:'#fff',border:'1px solid #E5DFD3',borderRadius:4}}>
        {flags.filter(f=>f.category===cat).map(f=><div key={f.flag} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'11px 16px',borderBottom:'1px solid #F0EBE0'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            <div onClick={()=>onToggle(f.flag,!f.enabled)} style={{width:40,height:22,borderRadius:11,background:f.enabled?'#2D5A3F':'#CCC',cursor:'pointer',position:'relative',transition:'background .2s',flexShrink:0}}>
...
function BookingModal({patient=null, medics, bookings=[], user, preselectedMedicId=null, prefillDate=null, prefillTime=null, onClose, onSaved}){
  ...
  const [patientPrenume, setPatientPrenume] = useState(patient?.prenume||'')
  ...
  const save = async () => {
    ...
    if(!patientPrenume.trim()) { setErr('Introduceți prenumele pacientului'); return }
    if(!patientEmail.trim()) { setErr('Introduceți adresa de email'); return }
    ...
    try {
      ...
      const body = {
        ...
        patient_prenume: patientPrenume.trim()||null,
      ...
      <div>
        ...
        {patient ? (
          ...
          <div style={{marginBottom:14}}>
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              <div><Lbl>Nume (familie) *</Lbl><Inp value={patientName} onChange={e=>setPatientName(e.target.value)}/></div>
              <div><Lbl>Prenume *</Lbl><Inp value={patientPrenume} onChange={e=>setPatientPrenume(e.target.value)}/></div>
...
// ── BookingEditModal ───────────────────────────────────────────────
function BookingEditModal({booking, medics, patients=[], onClose, onSaved, canApprove=true, canDelete=false, onViewPatient=null}){
  ...
  const selectPatient=p=>{
    fv('patient_id',p.id)
    fv('patient_name',p.name)
    fv('patient_email',p.email||'')
    fv('phone',p.phone||'')
    setPatSearch('')
  }
  ...
  const filteredPats=patients.filter(p=>{
    ...
    return(p.name||'').toLowerCase().includes(q)||(p.prenume||'').toLowerCase().includes(q)||(p.phone||'').includes(q)||(p.email||'').toLowerCase().includes(q)
  }).slice(0,20)

  const save=async()=>{
    setSaving(true);setErr('')
    const med=medics.find(m=>m.id===form.medic_id)
    const r=await fetch('/api/bookings',{method:'PUT',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({id:booking.id,...form,medic_name:med?.name||booking.medic_name})})
    if(!r.ok){const e=await r.json();setErr(e.error||'Eroare');setSaving(false);return}
    setSaving(false);onSaved()
  }

  return<SafeModal onClose={onClose} width={560}>
    {showAddPatient&&<SafeModal onClose={()=>setShowAddPatient(false)} width={600}>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,color:'#1A3A2A',marginBottom:16}}>Adaugă Pacient</div>
      <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E',marginBottom:16}}>Adăugați pacientul din secțiunea Pacienți din Dashboard, apoi reveniți la această programare.</div>
      <button className="btn-outline" style={{color:'#1A3A2A'}} onClick={()=>setShowAddPatient(false)}>Închide</button>
    </SafeModal>}
    <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,color:'#1A3A2A',marginBottom:16}}>Editează Programarea</div>
    {err&&<div style={{background:'#FFF3E0',border:'1px solid #FFCC80',padding:10,fontSize:12,color:'#E65100',marginBottom:10}}>{err}</div>}
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:10}}>
      <div style={{gridColumn:'1/-1'}}>
        <Lbl>Pacient</Lbl>
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          <Sel value={form.patient_id||''} onChange={e=>{
            const p=patients.find(x=>x.id===e.target.value)
            if(p){fv('patient_id',p.id);fv('patient_name',p.name);fv('patient_email',p.email||'');fv('phone',p.phone||'')}
            else fv('patient_id','')
          }} style={{flex:1}}>
            <option value="">— Alege pacient —</option>
            {patients.slice().sort((a,b)=>(a.name||'').localeCompare(b.name||'')).map(p=><option key={p.id} value={p.id}>{p.name}{p.prenume?` ${p.prenume}`:''}{p.phone?` · ${p.phone}`:''}</option>)}
          </Sel>
          {form.patient_id&&<button type="button" onClick={()=>{if(onViewPatient){const p=patients.find(x=>x.id===form.patient_id);if(p){onViewPatient(p)}}}} className="btn-outline" style={{color:'#2D5A3F',padding:'8px 12px',fontSize:11,whiteSpace:'nowrap'}} title="Mergi la profil">→ Profil</button>}
        </div>
        {form.patient_id&&<div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#6B7A6E',marginTop:4}}>{form.patient_name}{form.patient_email?` · ${form.patient_email}`:''}</div>}
      </div>
      <div><Lbl>Telefon</Lbl><Inp type="tel" value={form.phone} onChange={e=>fv('phone',e.target.value)}/></div>
      <div><Lbl>Email</Lbl><Inp type="email" value={form.patient_email} onChange={e=>fv('patient_email',e.target.value)}/></div>
      <div><Lbl>Medic</Lbl><Sel value={form.medic_id} onChange={e=>fv('medic_id',e.target.value)}><option value="">— Fără medic —</option>{medics.filter(m=>m.active).map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</Sel></div>
      <div><Lbl>Status</Lbl><Sel value={form.status} onChange={e=>fv('status',e.target.value)}>{['pending','approved','rejected','cancelled'].map(s=><option key={s} value={s}>{({pending:'În așteptare',approved:'Aprobat',rejected:'Respins',cancelled:'Anulat'})[s]}</option>)}</Sel></div>
      <div><Lbl>Data</Lbl><Inp type="date" value={form.appointment_date} onChange={e=>fv('appointment_date',e.target.value)}/></div>
      <div><Lbl>Ora</Lbl><Sel value={form.appointment_time} onChange={e=>fv('appointment_time',e.target.value)}>{ALL_HOURS.map(h=><option key={h}>{h}</option>)}</Sel></div>
      <div style={{gridColumn:'1/-1'}}><Lbl>Motiv</Lbl><Txt rows={2} value={form.reason} onChange={e=>fv('reason',e.target.value)}/></div>
    </div>
    {booking.status==='pending'&&canApprove&&<div style={{display:'flex',gap:8,padding:'10px 0',borderTop:'1px solid #F0EBE0',marginTop:8}}>
      <button onClick={async()=>{await fetch(`/api/bookings/approve?id=${booking.id}`);onSaved()}} style={{background:'#2D5A3F',color:'white',border:'none',padding:'8px 16px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:12,borderRadius:4}}>✓ Aprobă</button>
      <button onClick={async()=>{await fetch('/api/bookings/reject',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:booking.id})});onSaved()}} style={{background:'#8B3A3A',color:'white',border:'none',padding:'8px 16px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:12,borderRadius:4}}>✗ Respinge</button>
    </div>}
    <div style={{display:'flex',gap:10,marginTop:8,justifyContent:'space-between',alignItems:'center'}}>
      <div style={{display:'flex',gap:10}}>
        <button className="btn-primary" style={{display:'flex',alignItems:'center',gap:8}} onClick={save} disabled={saving}>{saving?<><Sp/> Salvează…</>:'Salvează'}</button>
        <button className="btn-outline" style={{color:'#1A3A2A'}} onClick={onClose}>Anulează</button>
      </div>
      {canDelete&&<button onClick={async()=>{if(!confirm('Ștergi programarea?'))return;await fetch('/api/bookings',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:booking.id})});onSaved()}} style={{background:'none',border:'1.5px solid #8B3A3A',padding:'7px 14px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:12,color:'#8B3A3A',borderRadius:4}}>✕ Șterge</button>}
    </div>
  </SafeModal>
}
...
// ── KanbanView ─────────────────────────────────────────────────────
function KanbanView({bookings, medics, patients=[], user, medicFlag, isAdmin, onRefresh, onViewPatient=null}){
  const [draggingId, setDraggingId] = useState(null)
  const [dragOverCol, setDragOverCol] = useState(null)
  const [saving, setSaving] = useState(false)
  const draggingBooking = draggingId ? bookings.find(b=>b.id===draggingId) : null

  const editBookingRef = useRef(null)
  const [editBooking, setEditBooking] = useState(null)

  const kanbanBookings = bookings.filter(b=>b.status!=='cancelled'&&b.status!=='rejected')
  const canMove = user?.role==='master'||user?.role==='admin'||(user?.role==='medic'&&medicFlag('bookings_reassign_medic'))
  const activeMedics = medics.filter(m=>m.active&&m.role!=='assistant')
  const columns = [{id:null,name:'Nealocat',color:'#6B7A6E'},...activeMedics.map(m=>({id:m.id,name:m.name,color:m.color||'#2D5A3F'}))]

  const getColBookings = medicId => kanbanBookings
    .filter(b=>medicId===null?!b.medic_id:b.medic_id===medicId)
    .sort((a,b)=>a.appointment_date.localeCompare(b.appointment_date)||a.appointment_time.localeCompare(b.appointment_time))

  const drop = async toMedicId => {
    if(!draggingBooking||!canMove) return
    const fromId = draggingBooking.medic_id||null
    if(fromId===toMedicId){setDraggingId(null);setDragOverCol(null);return}
    setSaving(true)
    const toMedic = toMedicId ? medics.find(m=>m.id===toMedicId) : null
    await fetch('/api/bookings',{method:'PUT',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({id:draggingBooking.id,medic_id:toMedicId||null,medic_name:toMedic?.name||null,status:'approved'})})
    setSaving(false)
    setDraggingId(null)
    setDragOverCol(null)
    onRefresh()
  ...
  const statusColor = s => s==='approved'?'#2D5A3F':s==='pending'?'#C9A84C':'#8B3A3A'

  return<div style={{display:'flex',flexDirection:'column',gap:12}}>
    {editBooking&&<BookingEditModal booking={editBooking} medics={medics} patients={patients} canApprove={true} canDelete={canMove} onViewPatient={p=>{setEditBooking(null);onViewPatient&&onViewPatient(p)}} onClose={()=>setEditBooking(null)} onSaved={()=>{setEditBooking(null);onRefresh()}}/>}
    {saving&&<div style={{padding:'8px 12px',background:'#EAF5EC',borderRadius:4,fontFamily:"'DM Sans',sans-serif",fontSize:12,color:'#2D5A3F'}}>Se salvează...</div>}
    {!canMove&&<div style={{padding:'8px 12px',background:'#FFF8E6',borderRadius:4,fontFamily:"'DM Sans',sans-serif",fontSize:12,color:'#7A6020'}}>Vizualizare — nu poți muta programările.</div>}
    <div style={{display:'flex',gap:12,overflowX:'auto',paddingBottom:8,alignItems:'flex-start'}}>
      {columns.map(col=>{
        const colBks = getColBookings(col.id)
        const isOver = dragOverCol===col.id
        const willReceive = isOver && draggingBooking && (draggingBooking.medic_id||null)!==col.id
        return<div key={String(col.id)}
          style={{minWidth:220,width:220,flexShrink:0,borderRadius:8,overflow:'hidden',border:`2px solid ${willReceive?col.color:'#E5DFD3'}`,background:willReceive?col.color+'0D':'#fff',transition:'border-color .1s, background .1s'}}
        ...
        >
          <div style={{padding:'10px 14px',background:col.color,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:500,color:'white'}}>{col.name}</div>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'rgba(255,255,255,.7)',background:'rgba(0,0,0,.15)',padding:'2px 8px',borderRadius:10}}>{colBks.length}</div>
          </div>
          <div style={{padding:8,display:'flex',flexDirection:'column',gap:5,minHeight:48}}>
            {willReceive&&<div style={{border:`2px dashed ${col.color}`,borderRadius:4,padding:'8px',fontFamily:"'DM Sans',sans-serif",fontSize:11,color:col.color,textAlign:'center',background:col.color+'0D'}}>Plasează aici</div>}
            {colBks.length===0&&!willReceive&&<div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#C0B8B0',textAlign:'center',padding:'12px 0'}}>Nicio programare</div>}
            {colBks.map(b=>{
              ...
              return<div key={b.id}
                ...
                style={{background:isDragging?'#F0EDE8':'#F9F7F4',border:'1px solid #E5DFD3',borderLeft:`4px solid ${statusColor(b.status)}`,borderRadius:4,padding:'8px 10px',cursor:canMove?'grab':'pointer',userSelect:'none',opacity:isDragging?0.4:1,transition:'opacity .15s',boxShadow:isDragging?'none':'0 1px 3px rgba(0,0,0,.06)'}}
              >
                <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:500,color:'#1A3A2A',marginBottom:3}}>{b.patient_name}</div>
                <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:10,color:'#6B7A6E',display:'flex',gap:6}}>
                  <span>{b.appointment_date}</span><span>{b.appointment_time}</span>
                  {b.status==='pending'&&<span style={{color:'#C9A84C',fontWeight:500}}>⏳</span>}
                </div>
              </div>
            })}
          </div>
...
function CalendarView({bookings,medics,user,medicFlag,canEdit,canDelete,canViewColleagues,onRefresh}){
  ...
  return<div style={{display:'flex',flexDirection:'column',height:'clamp(400px,calc(100vh - 160px),900px)',minHeight:400}}>
          ...
          {weekDays.map((d,di)=>{
            ...
            >
              ...
              {slotBookings.map((b,bi)=>{
                ...
                return<div key={b.id}
                  style={{position:'absolute',top:2,bottom:2,left:`${(bi/slotBookings.length)*100}%`,width:`${100/slotBookings.length}%`,background:statusColor,borderRadius:4,cursor:'pointer',overflow:'hidden',borderLeft:`4px solid ${medicCol}`}}
    ...
    {/* Tooltip */}
    {tooltip&&<div style={{position:'fixed',left:Math.min(tooltip.x+14,window.innerWidth-230),top:tooltip.y-8,background:'#1A3A2A',color:'#F5F0E8',padding:'10px 14px',borderRadius:4,zIndex:2000,fontFamily:"'DM Sans',sans-serif",fontSize:12,boxShadow:'0 4px 16px rgba(0,0,0,.25)',pointerEvents:'none',maxWidth:220}}>
    ...
    <div style={{display:'flex',gap:12,marginTop:8,flexWrap:'wrap',alignItems:'center'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:5}}>
        <div style={{width:10,height:10,borderRadius:4,background:'#E8F4E8',border:'1px solid #ccc'}}/>
...
// ── ExportPanel ────────────────────────────────────────────────────
function ExportPanel({patients, bookings, templates, user, medicFlag, isMedic}){
  const canExportPatients = user?.role==='master'||user?.role==='admin'||(isMedic&&medicFlag('export_patients'))
  const canExportBookings = user?.role==='master'||user?.role==='admin'||(isMedic&&medicFlag('export_bookings'))
  const canExportNomenclator = user?.role==='master'||user?.role==='admin'||(isMedic&&medicFlag('export_nomenclator'))
  const today = new Date().toISOString().split('T')[0]

  const dl=(data,filename,isJson)=>{
    if(!data.length)return
    let content,type
    if(isJson){content=JSON.stringify(data,null,2);type='application/json'}
    else{
  ...
  const patientRows=()=>patients.map(p=>({Nume:p.name||'',Prenume:p.prenume||'',CNP:p.cnp||'',CID:p.cid||'',Sex:p.sex||'',DataNasterii:p.data_nasterii||'',Email:p.email||'',Telefon:p.phone||'',Oras:p.oras||'',Judet:p.judet||'',Adresa:p.adresa||''}))
  const bookingRows=()=>bookings.map(b=>({Pacient:b.patient_name,Telefon:b.phone||'',Medic:b.medic_name||'',Data:b.appointment_date,Ora:b.appointment_time,Status:b.status,Motiv:b.reason||''}))
  const nomRows=()=>templates.map(t=>({Tip:t.type,Nume:t.name,Note:t.notes||''}))

  const ExportCard=({title,subtitle,csvFn,jsonData,disabled})=>(
    <div style={{background:'#fff',border:'1px solid #E5DFD3',padding:'20px 24px',borderRadius:8}}>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,color:'#1A3A2A',marginBottom:4}}>{title}</div>
      <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:'#6B7A6E',marginBottom:16}}>{subtitle}</div>
      <div style={{display:'flex',gap:10}}>
        <button onClick={()=>dl(csvFn(),'export.csv',false)} disabled={disabled} className="btn-primary" style={{padding:'8px 16px',fontSize:11}}>↓ CSV</button>
        <button onClick={()=>dl(jsonData,'export.json',true)} disabled={disabled} className="btn-outline" style={{color:'#1A3A2A',padding:'8px 16px',fontSize:11}}>↓ JSON</button>
      </div>
      {disabled&&<div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#8B3A3A',marginTop:8}}>Nu aveți permisiunea de export.</div>}
    </div>
  )

  return<div>
    <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:'#1A3A2A',marginBottom:6}}>Export Date</h2>
    <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E',marginBottom:24}}>Exportați datele în format CSV sau JSON.</p>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:16}}>
      <ExportCard title="Pacienți" subtitle={patients.length+' pacienți'} csvFn={patientRows} jsonData={patients} disabled={!canExportPatients}/>
      <ExportCard title="Programări" subtitle={bookings.length+' programări'} csvFn={bookingRows} jsonData={bookings} disabled={!canExportBookings}/>
      <ExportCard title="Nomenclator" subtitle="Simptome, diagnostice, tratamente" csvFn={nomRows} jsonData={templates} disabled={!canExportNomenclator}/>
    </div>
  </div>
}
...
function ImportPanel({user, onImported}){
  ...
  const handleFile=async e=>{
    ...
    try{
      const fd=new FormData();fd.append('file',file);fd.append('type','patients')
      const r=await fetch('/api/admin/import',{method:'POST',body:fd})
      const d=await r.json()
      if(!r.ok)throw new Error(d.error||'Eroare la import')
      setResult(d)
  ...
  const ComingSoon=({label})=>(
    <div style={{background:'#fff',border:'1px solid #E5DFD3',padding:'24px',borderRadius:8,textAlign:'center'}}>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,color:'#1A3A2A',marginBottom:8}}>{label}</div>
      <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E',background:'#F5F0E8',padding:'12px 20px',borderRadius:6,display:'inline-block'}}>În curând</div>
    </div>
  )

  return<div>
    <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:'#1A3A2A',marginBottom:6}}>Import Date</h2>
    <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E',marginBottom:24}}>Importați date în masă din fișiere Excel.</p>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:16}}>
      {/* Patients import */}
      <div style={{background:'#fff',border:'1px solid #E5DFD3',padding:'20px 24px',borderRadius:8}}>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,color:'#1A3A2A',marginBottom:4}}>Import Pacienți</div>
        <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:12,color:'#6B7A6E',marginBottom:4}}>Format Excel (.xlsx) — coloane: Nume, Prenume, CNP, CID, Sex, Data nașterii, Data decesului, Oraș, Județ, Adresă</div>
        <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#6B7A6E',marginBottom:14,padding:'8px 12px',background:'#F5F0E8',borderRadius:4}}>Prima linie trebuie să conțină antetele coloanelor. Pacienții existenți (după CNP) nu vor fi duplicați.</div>
        {err&&<div style={{background:'#FFF3E0',border:'1px solid #FFCC80',padding:'8px 12px',fontSize:12,color:'#E65100',marginBottom:10,borderRadius:4}}>{err}</div>}
        {result&&<div style={{background:result.errors>0?'#FFF8E6':'#EAF5EC',border:`1px solid ${result.errors>0?'#F0CC78':'#A5D6A7'}`,padding:'12px',fontSize:12,color:result.errors>0?'#7A6020':'#2D5A3F',marginBottom:10,borderRadius:4}}>
          <div style={{fontWeight:600,marginBottom:4}}>{result.errors>0?'⚠ Import finalizat cu erori':'✓ Import finalizat'}</div>
          <div>Importate: <strong>{result.imported}</strong> · Sărite (duplicate CNP): <strong>{result.skipped}</strong> · Erori: <strong>{result.errors}</strong> din <strong>{result.total}</strong> total</div>
          {result.errors>0&&<div style={{marginTop:6,fontSize:11,color:'#8B3A3A'}}>Rândurile cu erori au fost sărite. Verificați formatul datelor.</div>}
          {result.errorDetails?.length>0&&<ul style={{marginTop:4,paddingLeft:16,fontSize:11,color:'#8B3A3A'}}>{result.errorDetails.map((e,i)=><li key={i}>{e}</li>)}</ul>}
        </div>}
        <input type="file" ref={fileRef} accept=".xlsx" style={{display:'none'}} onChange={handleFile}/>
        <button onClick={()=>fileRef.current?.click()} disabled={importing} className="btn-primary" style={{padding:'8px 20px',fontSize:11,display:'flex',alignItems:'center',gap:8}}>
          {importing?<><Sp/> Se importă…</>:'↑ Alege fișier Excel'}
        </button>
      </div>
      <ComingSoon label="Import Vizite"/>
      <ComingSoon label="Import Servicii"/>
      <ComingSoon label="Import Nomenclator"/>
    </div>
  </div>
}
...
function AppInner(){
  ...
  const [patientSortDir,setPatientSortDir]=useState('asc')
  const patientSearchRef=useRef(null)
  // Computed filtered patients (used in both search header and PatientsList)
  const filtered=patients.filter(p=>{
    if(!patientSearch)return true
    const q=patientSearch.toLowerCase()
    return(p.name||'').toLowerCase().includes(q)||(p.prenume||'').toLowerCase().includes(q)||(p.phone||'').includes(q)||(p.email||'').toLowerCase().includes(q)||(p.cnp||'').includes(q)||(p.oras||'').toLowerCase().includes(q)
  }).sort((a,b)=>{
    const av=(a[patientSort]||'').toLowerCase()
    const bv=(b[patientSort]||'').toLowerCase()
  ...
  const [bookForm,setBookForm]=useState({name:'',prenume:'',email:'',phone:'',reason:''})
  ...
  const TAB_ICON = {
    ...
    export: (c='currentColor')=><svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke={c} strokeWidth="1.5"><path d="M10 3v10M6 9l4 4 4-4M4 15h12"/></svg>,
    import: (c='currentColor')=><svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke={c} strokeWidth="1.5"><path d="M10 17V7M6 11l4-4 4 4M4 5h12"/></svg>,
    cnas: (c='currentColor')=><svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke={c} strokeWidth="1.5"><rect x="3" y="3" width="14" height="14" rx="2"/><path d="M7 10h6M7 7h6M7 13h4"/></svg>,
    assistants: (c='currentColor')=><svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke={c} strokeWidth="1.5"><circle cx="8" cy="7" r="3"/><path d="M2 17c0-3 2.7-5 6-5"/><circle cx="15" cy="13" r="3"/><path d="M12 13h6M15 10v6"/></svg>,
    ai: (c='currentColor')=><svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke={c} strokeWidth="1.5"><circle cx="10" cy="10" r="8"/><path d="M7 10h6M10 7v6"/><circle cx="10" cy="10" r="2" fill={c} stroke="none"/></svg>,
  }
  ...
  const loadExtra=useCallback(async()=>{
    const[p,f,t]=await Promise.all([fetch('/api/admin/patients').then(r=>r.json()),fetch('/api/admin/flags').then(r=>r.json()),fetch('/api/admin/templates').then(r=>r.json()).catch(()=>[])])
    setPatients(Array.isArray(p)?p:[]);setFlags(Array.isArray(f)?f:[]);setTemplates(Array.isArray(t)?t:[])
  },[])
  ...
  const saveCrud=async (data, extras={})=>{
    ...
    if(editModel==='patients'&&(!data.name||!data.prenume)){throw new Error('Nume și Prenume sunt obligatorii')}
    ...
    const payload=editModel==='medics'?{...cleanMedic,staffPassword,staffRole:data.staffRole||'medic'}:editModel==='patients'?{...clean,password_hash:patientPassword||clean.password_hash}:clean
  ...
  const toggleFlag=async(f,e)=>{setFlags(prev=>prev.map(x=>x.flag===f?{...x,enabled:e}:x));await fetch('/api/admin/flags',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({flag:f,enabled:e})})}
  ...
  const adminTabs=()=>[
    ...
    ...(user?.role==='master'||user?.role==='admin'||medicFlag('assistants_view')?[['assistants','Asistenți']]:[] ),
    ...
    ...(user?.role==='master'||flag('admin_export')?[['export','Export Date']]:[] ),
    ...(user?.role==='master'||flag('admin_import')?[['import','Import Date']]:[] ),
    ...(user?.role==='master'||user?.role==='admin'?[['cnas','CNAS Sync']]:[] ),
    [['ai','Asistent AI']].flat(),
    ...(user?.role==='master'?[['flags','Setări Admin']]:[] ),
  ]

  const medicFlags=flags.filter(f=>f.flag.startsWith('medic_'))
  const adminFlagsList=flags.filter(f=>f.flag.startsWith('admin_')||f.flag.startsWith('medic_visits'))
  ...
  const medicTabs=()=>[
    ...
    ...(medicFlag('assistants_view')?[['assistants','Asistenți']]:[] ),
    ...(medicFlag('export_patients')||medicFlag('export_bookings')?[['export','Export Date']]:[] ),
    [['ai','Asistent AI']].flat(),
  ]
  ...
  const BookingsTable=({list,showApprove=false,showMedic=false,onCreateVisit=null,onEdit=null,onDelete=null})=><div style={{background:'#fff',border:'1px solid #E5DFD3',overflowX:'auto'}}>
    ...
    {list.map(b=><div key={b.id} style={{display:'grid',gridTemplateColumns:`90px 65px 1fr${showMedic?' 120px':''} 100px${(showApprove||onCreateVisit||onEdit||onDelete)?' 120px':''}`,gap:8,alignItems:'center',padding:'10px 14px',borderBottom:'1px solid #F0EBE0',fontFamily:"'DM Sans',sans-serif",fontSize:12,cursor:'pointer',minWidth:400}} onClick={()=>{if(onEdit)onEdit(b);else{const p=patients.find(x=>x.phone===b.phone||x.email===b.patient_email);if(p)setHistPatient(p)}}}>
      ...
      <div>
        <div style={{fontWeight:500,color:'#1A3A2A',display:'flex',alignItems:'center',gap:4}}>
          {b.patient_name}
          {b.patient_id&&<span onClick={e=>{e.stopPropagation();const p=patients.find(x=>x.id===b.patient_id||x.phone===b.phone||x.email===b.patient_email);if(p)setHistPatient(p)}} style={{fontSize:9,color:'#4A8C5C',cursor:'pointer',textDecoration:'underline',letterSpacing:'.04em'}}>profil</span>}
        </div>
      ...
      <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
        ...
        {onCreateVisit&&b.status==='approved'&&<button onClick={e=>{e.stopPropagation();onCreateVisit(b)}} style={{background:'none',border:'1px solid #2D5A3F',padding:'3px 8px',cursor:'pointer',fontSize:10,color:'#2D5A3F',borderRadius:4,whiteSpace:'nowrap'}}>+ Vizită</button>}
        {onEdit&&<button onClick={e=>{e.stopPropagation();onEdit(b)}} style={{background:'none',border:'1px solid #C9A84C',padding:'3px 8px',cursor:'pointer',fontSize:10,color:'#C9A84C',borderRadius:4}}>✎</button>}
        {onDelete&&<button onClick={e=>{e.stopPropagation();onDelete(b)}} style={{background:'none',border:'1px solid #8B3A3A',padding:'3px 8px',cursor:'pointer',fontSize:10,color:'#8B3A3A',borderRadius:4}}>✕</button>}
      </div>
    </div>)}
  ...
  const PatientsList=({canAdd,canEdit,canDel,canAddVisit,showVisitBtn=false,canBookForPatient=false,onBookForPatient})=>{
    return<div style={{background:'#fff',border:'1px solid #E5DFD3',borderRadius:4}}>
      {filtered.map(p=><div key={p.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 14px',borderBottom:'1px solid #F0EBE0',fontFamily:"'DM Sans',sans-serif",fontSize:13,flexWrap:'wrap',gap:8}}>
        <div style={{flex:1,cursor:'pointer',minWidth:0}} onClick={()=>setHistPatient(p)}>
          <div style={{fontWeight:500,color:'#1A3A2A',display:'flex',alignItems:'center',gap:6}}>{p.name}{p.prenume?` ${p.prenume}`:''}<span style={{fontSize:10,color:'#4A8C5C'}}>→ Istoric</span></div>
          <div style={{fontSize:11,color:'#6B7A6E'}}>{[p.phone,p.email,p.oras,p.cnp].filter(Boolean).join(' · ')}</div>
        </div>
  ...
  const SidebarNav=({tabs,current,onClick,tabIcons})=>{
    return<div style={{background:'#1A3A2A',flexShrink:0,display:'flex',flexDirection:'column',width:'clamp(160px,18vw,220px)',minHeight:'calc(100vh - 60px)',overflowX:'visible',}}>
      <div style={{display:'flex',flexDirection:'column',flex:1}}>
        {tabs.map(([t,l])=>{
          const label=l
          const Icon=tabIcons?.[t]
          const isActive=current===t
          return<div key={t} onClick={()=>onClick(t)} style={{padding:'10px 18px',cursor:'pointer',display:'flex',alignItems:'center',gap:10,background:isActive?'rgba(255,255,255,.1)':'transparent',borderLeft:isActive?'3px solid #C9A84C':'3px solid transparent',borderBottom:'none',transition:'all .15s',flexShrink:0,whiteSpace:'nowrap'}}>
      ...
      {<div style={{padding:'16px 18px',borderTop:'1px solid rgba(255,255,255,.1)'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <div style={{width:32,height:32,borderRadius:'50%',background:user?.medics?.color||'#C9A84C',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'DM Sans',sans-serif",fontSize:12,fontWeight:500,color:'#1A3A2A',flexShrink:0}}>
  ...
  return<div style={{fontFamily:"'Georgia',serif",background:'#F5F0E8',minHeight:'100vh',color:'#1A1A1A',display:'flex',flexDirection:'column'}}>
    ...
    {page==='home'&&<div className="fade-in">
      ...
      <div style={{background:'linear-gradient(90deg,#F0E4C0,#EDD99A,#F0E4C0)',display:'flex',justifyContent:'center',flexWrap:'wrap'}}>
        {[[medics.filter(m=>m.active&&m.role!=='assistant').length||'—','Medici'],[medics.filter(m=>m.active&&m.role==='assistant').length||'—','Asistenți'],['20+','Ani'],['15k+','Pacienți'],[services.filter(s=>s.active!==false).length||'—','Servicii']].map(([n,l])=>(
    ...
    {page==='team'&&<div className="fade-in" style={{maxWidth:1100,margin:'0 auto',padding:'clamp(28px,6vw,64px) clamp(16px,5vw,40px)'}}>
      ...
      <div style={{marginBottom:32}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(clamp(260px,42vw,400px),1fr))',gap:'clamp(12px,2vw,18px)'}}>
          {medics.filter(m=>m.active&&m.role!=='assistant').map(doc=>(
          <div key={doc.id} className="card" style={{padding:'24px 20px',cursor:'pointer'}} onClick={()=>{setSelMedic(doc);setPage('doctor')}}>
            {/* Doctor services badges */}
            {doc.medic_services?.length>0&&<div style={{display:'flex',flexWrap:'wrap',gap:4,marginBottom:10}}>
              {doc.medic_services.map(ms=><span key={ms.service_id} style={{background:'#F0E4C0',color:'#1A3A2A',fontFamily:"'DM Sans',sans-serif",fontSize:10,padding:'2px 8px',borderRadius:10}}>{ms.services?.name}</span>)}
            </div>}
      ...
      {/* Asistenți Medicali */}
      {medics.filter(m=>m.active&&m.role==='assistant').length>0&&<div>
        <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,letterSpacing:'.22em',textTransform:'uppercase',color:'#6B7A6E',marginBottom:8}}>Asistenți Medicali</div>
        <Rule/>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(clamp(240px,38vw,360px),1fr))',gap:'clamp(10px,2vw,16px)',marginTop:16}}>
          {medics.filter(m=>m.active&&m.role==='assistant').map(a=>(
            <div key={a.id} className="card" style={{padding:'20px',display:'flex',gap:14,alignItems:'flex-start'}}>
              <div style={{width:44,height:44,borderRadius:'50%',background:a.color||'#3F4A5C',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'DM Sans',sans-serif",fontSize:14,fontWeight:500,color:'white',flexShrink:0}}>
                {getInitials({name:a.name})}
              </div>
              <div>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:600,color:'#1A3A2A'}}>{a.name}</div>
                <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#C9A84C',letterSpacing:'.1em',textTransform:'uppercase',marginTop:2}}>{a.specialization||'Asistent Medical'}</div>
                {a.email&&<div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#6B7A6E',marginTop:4}}>{a.email}</div>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{maxWidth:960,margin:'0 auto',padding:'clamp(18px,4vw,48px) clamp(16px,5vw,40px)',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:'clamp(16px,3vw,28px)'}}>
        ...
        <div>
          ...
          {bookDone?<div style={{background:'#EAF5EC',border:'1px solid #A8D4AD',padding:'24px',textAlign:'center'}}>
            ...
            {!patient&&<>
              {[['Nume (familie) *','name','text'],['Prenume *','prenume','text'],['Email *','email','email'],['Telefon','phone','tel']].map(([l,k,t])=>(
            ...
            <button className="btn-primary" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8}} onClick={book}
              disabled={!selSlot||!dk||!captcha||((!patient)&&(!bookForm.name||!bookForm.prenume||!bookForm.email))||saving}>
          ...
          <div style={{marginTop:24,padding:'18px',background:'#fff',border:'1px solid #E5DFD3'}}>
            ...
            {[['Adresă:',settings.clinic_address],['Tel:',settings.clinic_phone],['Email:',settings.clinic_email],['Program:',settings.clinic_hours]].filter(([,v])=>v).map(([i,t])=>(
    ...
    {page==='pdash'&&patient&&<div className="fade-in">
      ...
      :<div style={{maxWidth:900,margin:'0 auto',padding:'clamp(16px,4vw,56px) clamp(12px,4vw,40px)'}}>
        ...
        <div style={{display:'flex',borderBottom:'2px solid #E5DFD3',marginBottom:18,overflowX:'auto',flexShrink:0,scrollbarWidth:'none'}}>
    ...
    {page==='dashboard'&&user&&<div className="fade-in">
      ...
      {!histPatient&&<div style={{display:'flex',flex:1,flexDirection:'row',minHeight:'calc(100vh - 60px)'}}>
        ...
        <div style={{flex:1,padding:'clamp(12px,3vw,40px)',overflowY:'auto',minWidth:0,maxWidth:'100%',minHeight:0}}>
          ...
          {adminTab==='bookings'&&<div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexWrap:'wrap',gap:10}}>
              ...
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                {/* View toggle */}
                <div style={{display:'flex',border:'1px solid #E5DFD3',borderRadius:4,overflow:'hidden'}}>
                  {[['list','list'],['calendar','calendar'],['kanban','kanban']].map(([v,l])=>{
                    ...
                    return<div key={v} onClick={()=>setBookingsView(v)} style={{padding:'6px 12px',cursor:'pointer',display:'flex',alignItems:'center',gap:6,background:isActive?'#1A3A2A':'transparent',color:isActive?'#F5F0E8':'#6B7A6E'}}>
                      {v==='list'
                        ...
                        :v==='calendar'?<svg viewBox="0 0 16 16" width="13" height="13" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="1" y="3" width="14" height="12" rx="1"/><path d="M1 7h14M5 1v4M11 1v4"/></svg>:<svg viewBox="0 0 16 16" width="13" height="13" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="1" y="2" width="4" height="12" rx="1"/><rect x="6" y="2" width="4" height="8" rx="1"/><rect x="11" y="2" width="4" height="10" rx="1"/></svg>
                      }
                      <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11}}>{v==='list'?'Listă':v==='calendar'?'Calendar':'Panou'}</span>
                    </div>
                  })}
                </div>
                <Refresh/>
            ...
            {bookingsView==='kanban'&&(user?.role==='master'||user?.role==='admin'||(isMedic&&medicFlag('bookings_reassign_medic')))&&<KanbanView
              bookings={bookings}
              medics={medics}
              patients={patients}
              user={user}
              ...
              isAdmin={!isMedic}
              onRefresh={load}
              onViewPatient={p=>setHistPatient(p)}
            />}
            {bookingsView==='kanban'&&isMedic&&!medicFlag('bookings_reassign_medic')&&<div style={{padding:32,textAlign:'center',fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E'}}>Nu aveți permisiunea de a accesa Panoul Kanban.</div>}
          ...
          {adminTab==='patients'&&(histPatient
            ...
            :<div>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14,flexWrap:'wrap',gap:8}}>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:26,color:'#1A3A2A'}}>Pacienți ({filtered.length}/{patients.length})</div>
                <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
                  <input
                    className="input-field"
                    placeholder="Caută (nume, telefon, CNP, oraș...)"
                    onChange={e=>setPatientSearch(e.target.value)}
                    style={{width:'clamp(140px,25vw,220px)',padding:'8px 12px'}}
                  />
                  <Sel value={patientSort} onChange={e=>setPatientSort(e.target.value)} style={{width:'auto'}}>
                    <option value="name">Sortare: Nume</option>
                    <option value="prenume">Prenume</option>
                    <option value="oras">Oraș</option>
                    <option value="created_at">Dată</option>
                  </Sel>
                  <button onClick={()=>setPatientSortDir(d=>d==='asc'?'desc':'asc')} className="btn-outline" style={{color:'#1A3A2A',padding:'8px 10px'}}>{patientSortDir==='asc'?'↑':'↓'}</button>
                  <Refresh/>
                  {(user?.role==='master'||(isMedic?medicFlag('patients_add'):flag('admin_patients_add')))&&<button className="btn-primary" onClick={()=>openModal('patients')}>+ Adaugă</button>}
                </div>
              </div>
              <PatientsList
          ...
          {/* ASSISTANTS */}
          {adminTab==='assistants'&&<div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexWrap:'wrap',gap:10}}>
              <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:26,color:'#1A3A2A'}}>Asistenți ({medics.filter(m=>m.role==='assistant').length})</h2>
              <div style={{display:'flex',gap:8}}>
                <Refresh/>
                {(user?.role==='master'||user?.role==='admin'||medicFlag('assistants_manage'))&&<button className="btn-primary" onClick={()=>openModal('medics',{role:'assistant',staffRole:'assistant'})}>+ Adaugă Asistent</button>}
              </div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:12}}>
              {medics.filter(m=>m.role==='assistant').map(a=>(
                <div key={a.id} className="card" style={{padding:'18px 20px',display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                  <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
                    <div style={{width:40,height:40,borderRadius:'50%',background:a.color||'#3F4A5C',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:500,color:'white',flexShrink:0}}>{getInitials({name:a.name})}</div>
                    <div>
                      <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:500,color:'#1A3A2A',fontSize:14}}>{a.name}</div>
                      {a.specialization&&<div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#C9A84C',marginTop:2}}>{a.specialization}</div>}
                      <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,color:'#6B7A6E',marginTop:2}}>{a.email}</div>
                    </div>
                  </div>
                  <div style={{display:'flex',gap:6,flexShrink:0}}>
                    {(user?.role==='master'||user?.role==='admin'||medicFlag('assistants_manage'))&&<button className="btn-outline" style={{color:'#1A3A2A',padding:'3px 9px',fontSize:10}} onClick={()=>openModal('medics',a)}>Edit</button>}
                  </div>
                </div>
              ))}
              {medics.filter(m=>m.role==='assistant').length===0&&<div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E',padding:16}}>Niciun asistent înregistrat.</div>}
            </div>
          </div>}

          {/* AI ASSISTANT */}
          {adminTab==='ai'&&<div>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:'#1A3A2A',marginBottom:6}}>Asistent AI</h2>
            <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E',marginBottom:24}}>Asistent inteligent integrat în fluxul de lucru al cabinetului.</p>
            <div style={{background:'#fff',border:'1px solid #E5DFD3',borderRadius:8,padding:48,textAlign:'center'}}>
              <div style={{marginBottom:16,display:'flex',justifyContent:'center'}}>
                <svg viewBox="0 0 48 48" width="48" height="48" fill="none" stroke="#1A3A2A" strokeWidth="1.5">
                  <circle cx="24" cy="24" r="20"/><path d="M24 16v8l5 5"/><circle cx="24" cy="24" r="3" fill="#C9A84C" stroke="none"/>
                  <path d="M16 8l2 4M32 8l-2 4M16 40l2-4M32 40l-2-4"/>
                </svg>
              </div>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,color:'#1A3A2A',marginBottom:8}}>În Curând</div>
              <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E'}}>Asistentul AI va fi disponibil într-o versiune viitoare.</div>
            </div>
          </div>}
          ...
          {/* EXPORT */}
          {adminTab==='export'&&<ExportPanel patients={patients} bookings={isMedic?bookings.filter(b=>b.medic_id===user.medic_id):bookings} templates={templates} user={user} medicFlag={medicFlag} isMedic={isMedic}/>}

          {/* IMPORT */}
          {adminTab==='import'&&<ImportPanel user={user} onImported={()=>{loadExtra();load()}}/>}

          {/* ADMIN FLAGS (Master only) */}
          {adminTab==='cnas'&&<div>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:'#1A3A2A',marginBottom:6}}>CNAS Sync</h2>
            <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E',marginBottom:24}}>Sincronizare cu Casa Națională de Asigurări de Sănătate.</p>
            <div style={{background:'#fff',border:'1px solid #E5DFD3',borderRadius:8,padding:'48px',textAlign:'center'}}>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,color:'#1A3A2A',marginBottom:8}}>În Curând</div>
              <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:'#6B7A6E'}}>Integrarea CNAS va fi disponibilă într-o versiune viitoare.</div>
            </div>
          </div>}
          {adminTab==='flags'&&user.role==='master'&&<FlagsEditor flags={adminFlagsList} onToggle={toggleFlag} title="Setări Admin" subtitle="Controlează ce pot face Adminii."/>}
    ...
    {modal&&<CrudModal model={editModel} item={editItem} currentUserRole={user?.role||''} onClose={()=>{setModal(false);setEditItem(null)}} onSave={saveCrud} services={services}/>}
